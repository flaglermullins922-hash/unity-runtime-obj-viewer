...
![程序运行画面](Docs/video_frames_grid.png)

*上图为程序运行时的 6 个画面（自左向右、自上而下）：
材质色环绕 → 顶点高度渐变 → 顶点云 → 顶点呼吸动画 → 线框 → 法线可视化。*

> 演示视频由本人录屏提交，不含在本仓库内；仓库只保留程序、说明与真实渲染出的截图。录屏脚本见 [第七节](#七运行演示与录屏)。
...
| 滚轮 | 缩放 |
| `←` `↑` `↓` `→` | 旋转（鼠标不可用时的备用方案） |
| 信息面板上的按钮 | 左转/右转/抬高/降低/放大/缩小/自动旋转/重置 —— 完全不依赖鼠标拖拽 |
...
> **关于输入实现**：相机输入一律读取 `Input.mousePosition` 的逐帧差值与 `Input.mouseScrollDelta`，
> **不使用 `Input.GetAxis("Mouse X"/"Mouse Y")`**。因为 `GetAxis` 依赖 `ProjectSettings/InputManager.asset`
> 里的轴定义，一旦轴缺失或被改动，它会**静默返回 0 且不报错**，表现就是「按住拖拽毫无反应」。
> `mousePosition` 是原始数据，不受任何轴配置影响。
>
> 信息面板顶部还内置了**输入自检**（`鼠标在 Game 视图内` / `已识别到拖拽` / 实时鼠标位移），
> 万一拖不动，看一眼这两行就能判断是「鼠标不在 Game 窗口里」还是「输入没被接收」。
...
演示视频由本人录屏提交，**仓库内不含视频文件**。

### 20 秒演示脚本（进入 Play 后照这个顺序操作）

| 时间 | 键盘 / 鼠标操作 | 画面效果 |
|---|---|---|
| 0 – 4 s | 左键按住，横向慢慢拖拽 | 相机环绕，展示模型整体造型 |
| 4 – 7 s | 按 `2` | 切到**顶点高度渐变**：每个顶点按 Y 高度着色（青→紫→橙） |
| 7 – 11 s | 按 `空格` 打开顶点云，滚轮推近 | 模型表面出现**每个顶点一个小方块** |
| 11 – 14.5 s | 再按 `空格` 关掉顶点云，按 `V` | **顶点呼吸动画**：沿法线轻微起伏，证明顶点可写 |
| 14.5 – 17.5 s | 按 `V` 停止，按 `4`，滚轮推近 | **线框**：能看清三角剖分（该模型 49,489 个三角面） |
| 17.5 – 20 s | 按 `3`，滚轮拉远 | **法线可视化**收尾 |

> 左上角信息面板会实时显示顶点数、三角面数、解析耗时、前三个顶点坐标等数据，录进去更有说服力；按 `Tab` 可收起。

### 录屏方法

| 方法 | 操作 | 备注 |
|---|---|---|
| **Xbox Game Bar**（推荐，系统自带） | `Win + G` → 点录制按钮 → 结束按 `Win + Alt + R` | 录像存于「视频\Captures」 |
| 剪映 | 录制 → 裁剪到 20 秒内导出 | 可顺带加字幕，本机已装 |
| OBS | 需另装 | 画质/码率可控性最好 |
...
仓库内还带了 `DemoVideoRecorder.cs`，它能在 Unity 里**逐帧渲染**（不是录屏）后用 ffmpeg 合成 20 秒视频，
帧率绝对稳定、可重复生成。需要时运行：
...
输出为 `Docs/demo_20s.mp4`（约 18 MB）。若想让 GitHub 网页直接在线播放，需压到 10 MB 以内：
`ffmpeg -i demo_20s.mp4 -c:v libx264 -crf 26 -pix_fmt yuv420p demo_lite.mp4`（约 7 MB，肉眼无差）。
