...
![演示预览：六个关键帧](Docs/video_frames_grid.png)

*上图为 [`Docs/demo_20s.mp4`](Docs/demo_20s.mp4) 的 6 个关键帧（自左向右、自上而下）：
...
| 演示视频 | [`Docs/demo_20s.mp4`](Docs/demo_20s.mp4)（20.0 秒 / 1280×720 / 30 fps / 7.2 MB，**GitHub 网页可直接播放**） |
...
> 为方便 **GitHub 网页直接在线播放**，仓库内的版本压到了 7.2 MB（H.264 CRF 26，画质与原始高码率版肉眼无差）。
> 需要更高码率时，重新跑一次录制脚本即可（默认输出约 18 MB）。
